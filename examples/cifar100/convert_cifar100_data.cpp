...
const int kCIFARTestImagese = 10000;

void read_image(std::ifstream* file, int* label, char* buffer) {
  char label_char;
  file->read(&label_char, 1);	// coarse label
...
void convert_dataset(const string& input_folder, const string& output_folder) {
  ...
  status = leveldb::DB::Open(options, output_folder + "/cifar100_train_leveldb",
      &train_db);
  CHECK(status.ok()) << "Failed to open leveldb.";
  for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    // Open files
    LOG(INFO) << "Training Batch " << fileid + 1;
    snprintf(str_buffer, kCIFARImageNBytes, "/train.bin", fileid + 1);
  ...
  for (int itemid = 0; itemid < kCIFARTestImagese; ++itemid) {
    read_image(&data_file, &label, str_buffer);
    datum.set_label(label);
    datum.set_data(str_buffer, kCIFARImageNBytes);
    datum.SerializeToString(&value);
