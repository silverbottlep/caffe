...
template<typename Dtype>
void DataTransformer<Dtype>::TransformVideo(const int batch_item_id,
                                       const Datum& datum,
                                       const Dtype* mean,
  ...
  const Dtype scale = param_.scale();
	...
	int start_frame = 0;
	if (num_samples>0){
		start_frame = Rand()%(datum.channels()-num_samples-1);
  ...
  if (crop_size) {
    ...
    if (phase_ == Caffe::TRAIN) {
      ...
      w_off = (width - crop_size) / 2;
		...
		LOG(INFO) << " start_frame: " << start_frame << " end_frame:" << start_frame+channels << " data_channels: " << datum.channels() << " imagesize: " << height << ":" << width;
    if (mirror && Rand() % 2) {
      // Copy mirrored version
      for (int c = start_frame; c < start_frame+channels; ++c) {
      //for (int c = 0; c < channels; ++c) {
        for (int h = 0; h < crop_size; ++h) {
          for (int w = 0; w < crop_size; ++w) {
            int data_index = (c * height + h + h_off) * width + w + w_off;
